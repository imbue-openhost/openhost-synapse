...
The bundled Cinny client is compiled from source in a `node` builder stage in the `Dockerfile`, and `cinny-suppress-connecting-banner.patch` is applied before `npm run build`. It is pinned two ways: `CINNY_VERSION` is the human-readable release tag used to clone, and `CINNY_COMMIT_SHA` is the immutable commit that tag must resolve to. Because a git tag is a mutable ref (it can be force-moved upstream), the build verifies the checked-out `HEAD` against `CINNY_COMMIT_SHA` and aborts on any mismatch. When bumping the version, update both values (and regenerate the patch if needed).
...
The build guards against silent regressions: the clone step fails if the tag resolves to a commit other than the pinned `CINNY_COMMIT_SHA`, `git apply --check` fails if the patch no longer applies (e.g. after a `CINNY_VERSION` bump), and a post-build check fails if the "Connecting..." string is still present or if the error banners were removed. If a version bump breaks the patch, regenerate it against the new tag.
